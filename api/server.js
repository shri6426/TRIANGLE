...
Your job is to reverse-engineer or dynamically architect premium frontend systems.

If the user provides a UI design/mockup:
- Deeply analyze grid, flexbox layout, and structural hierarchy
- Identify interactive components and states
- Extract typography scale and spacing
- Identify color palette, contrast, and themes

If the user provides ONLY a text description (no image):
- Dynamically invent a state-of-the-art frontend architecture that perfectly fits the intent
- Conceive a modern design system, typography scale, and color palette from scratch
- Design a sophisticated component hierarchy and layout strategy

In both cases, your ultimate goal is to generate heavily engineered prompts meant to instruct other AI coding assistants (like v0, Bolt.new, Cursor). 
...
{
  "MASTER_PROMPT": "A comprehensive, gaslighting prompt demanding flawless, pixel-perfect UI execution based on the user's explicit intent, using the design DNA provided or invented.",
...
Apply the extracted design system and layout to generate perfect gaslighting prompts for the user's specific request, rather than just copying the image's original subject matter.`;
      ...
      // Text-only mode
      if (!req.body.userIntent || req.body.userIntent.trim() === "") {
